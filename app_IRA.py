...
for i in range(6):
    ...
    # 1. Primeiro criamos a caixa de seleção da matéria
    with col_nome:
        materia_selecionada = st.selectbox("", LISTA_MATERIAS, key=f"m_{i}", label_visibility="collapsed")
    
    # 2. Buscamos a carga horária correta com base na seleção atual
    ...
    with col_cred:
        if materia_selecionada == "Outra":
            # Libera o campo para digitação caso seja "Outra"
            hor = st.number_input("", min_value=0, max_value=200, value=0, step=20, key=f"c_{i}", label_visibility="collapsed")
        else:
            # Força o valor correto diretamente no estado do Streamlit para atualizar na tela
            st.session_state[f"c_{i}"] = hor_fixo
            hor = st.number_input("", min_value=0, max_value=200, key=f"c_{i}", label_visibility="collapsed", disabled=True)
